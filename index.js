...
function getRoles() {
    ...
    const dest = document.getElementById("roles");

    try {
        const result = processJwt(text);
        dest.innerHTML = result?.claimsJson?.roles?.join('<br>');
    } catch (error) {
        dest.innerHTML = error.message;
    }
}
